        ...
        <div key={`${petId}-${petBounce}`} className="relative h-full w-full">
          {isSiamese ? (
            <video
              ...
              className="absolute left-1/2 top-1/2 h-[95%] w-full -translate-x-1/2 -translate-y-1/2 object-contain animate-float-slow drop-shadow-2xl"
              style={{ background: "transparent" }}
            />
          ) : (
            <img src={pet.images.idle} alt={pet.name} draggable={false}
              className="absolute left-1/2 top-1/2 h-[90%] w-full -translate-x-1/2 -translate-y-1/2 object-contain animate-float-slow drop-shadow-2xl" />
          )}
        </div>
